...
def create_audit_log():
    ...
    action = (payload.get("action") or "").strip() or "unknown"

    log = AuditLog(action=action, created_at=utc_now_iso())
    log.save()

    return envelope(data={"id": str(log.id), "action": log.action, "created_at": log.created_at}, http_code=201)
